 ...
 * @author ahl
 * @desc
 * @create 2023/6/19 15:55
 */
@Slf4j
public class PdfUtils {

    public static String readTextAndTable(File file) {
...
//                tableList.addAll(tableTempList);
            }
            pd.close();
        } catch (IOException e) {
        ...
        //表合并
//        List<Map<String, Object>> newTableList = new ArrayList<>();
//        Map<String ,Object> preTable = null;
//        for (Map<String ,Object> table: tableList) {
//            if (preTable != null) {
//                if (preTable.get("header").equals(table.get("header")) && (StringUtils.isEmpty(table.get("title").toString()) || preTable.get("title").equals(table.get("title")))) {
//                    List<String> splits = Arrays.asList(preTable.get("table").toString().split("\n"));
//                    String tableStr = preTable.get("table").toString();
//                    for (String split : splits) {
//                        if(tableStr.contains(split)) {
//                            continue;
//                        }
//                        tableStr += split;
//                    }
//                    table.put("table", tableStr);
//                    if (StringUtils.isNotEmpty(preTable.get("header").toString()) && StringUtils.isEmpty(table.get("header").toString())){
//                        table.put("header", preTable.get("header"));
//                    }
//                    if (StringUtils.isNotEmpty(preTable.get("other").toString()) && StringUtils.isEmpty(table.get("other").toString())){
//                        table.put("other", preTable.get("other"));
//                    }
//                } else {
//                    newTableList.add(preTable);
//                }
//            }
//            preTable = table;
//        }
//        result.put("text", textInfo.toString());
//        result.put("table", newTableList);
